...
// Defines de las direcciones base físicas de los dos dispositivos que controlan el LCD
...
// Ojo habrá que modificar los valores de BaseAddress para que correspondan con las direcciones virtuales
...
void LCD_SetCursor(alt_u16 Xpos, alt_u16 Ypos)
{
	 LCD_WR_REG(0x002A);	//Comando Column Address Set (Columna X)
		 LCD_WR_DATA(Xpos>>8);	// Se envía  1 byte (Parte Alta del valor de la columna)
		 LCD_WR_DATA(Xpos&0XFF); // Se envía 1 byte (Parte Baja del valor de la columna)
...
void LCD_DrawPoint(alt_u16 x,alt_u16 y,alt_u16 color )
{
        LCD_SetCursor(x,y); // Posición X,Y
        LCD_WR_REG(0x002C);	// Comando Memory Write
        LCD_WR_DATA(color);	// Datos de la memoria (Color del pixel)
}
